...
for ntrial in trials:
    ...
    for case in my_list:
        ...
        for npyr in xrange(Npyramidals):
            ...
            sizetest1 = field_size(rate_map, relfreq=0.2*maxpeak, track_length=Nbins)[0]
            ...
            clevel = 0.99
            if (maxpeak>=1.0) and (pval_infor>=clevel) and (pval_staball>=clevel) and (5./(npath_x/Nbins)<=sizetest1<=25./(npath_x/Nbins)):# and (npyr in place_cells):#<=50./(npath_x/Nbins):
                ...
                stabALL2.append((np.math.atanh(stab1[0])+np.math.atanh(stab2[0]))/2.0)
                ...
                average1ALL.append(field_size(rate_map, relfreq=0.2*np.max(rate_map), track_length=Nbins)[1])
                average2ALL.append(field_size(rate_map, relfreq=0.2*np.max(rate_map), track_length=Nbins)[2])
                coher2.append(sp_coher)
            
            inforALL1.append(infor[0])
            stabALL1.append((np.math.atanh(stab1[0])+np.math.atanh(stab2[0]))/2.0)
        ...
        plt.tight_layout()
        plt.savefig(path_figs+'/'+case+'_PlaceCells_heatmap_'+str(ntrial)+'.pdf',format='pdf',dpi=300)
        plt.cla()
        plt.clf()
        plt.close()
...
plt.savefig(path_figs2+'/'+'numberOfPlaceCells_barplot.eps',format='eps',dpi=300)
plt.savefig(path_figs2+'/'+'numberOfPlaceCells_barplot.png',format='png',dpi=300)
plt.cla()
plt.clf()
plt.close()
...
bplot = plt.boxplot(y, notch=False, vert=True, patch_artist=True, labels=labels)  # will be used to label x-ticks
...
plt.ylabel('number of place cells', fontsize=16)
plt.savefig(path_figs2+'/'+'numberOfPlaceCells_boxplot.eps',format='eps',dpi=300)
plt.savefig(path_figs2+'/'+'numberOfPlaceCells_boxplot.png',format='png',dpi=300)
plt.cla()
plt.clf()
plt.close()
