...
def extract_spiketimes(neuron_type, condition, ntrial, nrun):
    ...
    '''
    dt = 0.1
    filepath = '../Simulation_Results/'+condition + \
        '/Trial_'+str(ntrial)+'/Run_'+str(nrun)
    ...
    for n_neuron in range(Nneurons):
        ...
        filename = filepath+'/Trial_'+ntrial+'_Run_' + \
            nrun+neuron_type+str(n_neuron)+'.dat'
...
print("\nEverything was ok for pyramidals. Case: " +
      cond+" "+str(trial)+" "+str(run))
time2 = time.time()
