...
folderpath = 'Inputs_linear_rand_stops_noisy_jitter'+name+'_'+desync_factor+'/' #prelearning io same neg pos
...
print 'Factor '+desync_factor+' RUN '+str(nrun)+' ...\n'

p='run_' + str(nrun) + '/'
source_ec  = 'runs_produced_by_python_ec_rand_stops'+'/' + p  # prelearning, import ec input from io (same neg pos)
source_ca3 = 'runs_produced_by_python_ca3_rand_stops'+'/' + p # prelearning,  ...
dest=folderpath + p
...
for plf in range(1,n_place_fields+1):
    listdirs.append('place_field_'+str(plf))
...
for mydir in listdirs:
    ...
    for i in range(L_ec):
        ...
        with open(source_ec+mydir+'/s'+str(vec_ec[i])+'.txt', 'r') as f:
        ...
        if jitter=='EC':
            ...
            jitter       = list(np.round(int(desync_factor)*np.random.randn(len(lines_noisy)),1))
        ...
        with open(dest+'g'+str(counter_ec)+'_EC.txt', 'w') as f:
...
for mydir in listdirs:
    ...
    for i in range(0,L_ca3):
        ...
        with open(source_ca3+mydir+'/c'+str(vec_ca3[i])+'.txt', 'r') as f:
            lines = f.read().splitlines()
        ...
        with open(dest+'g'+str(counter_ca3)+'_CA3.txt', 'w') as f:
            for line in lines_noisy:
                nline=str(int(line))
                f.write(nline + '\n')
